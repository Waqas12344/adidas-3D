...
export function SecondModel({ shirtType }: { shirtType: ShirtType }) {
  ...
  useGSAP(() => {
    if(!marqueeText1Ref.current || !marqueeText1DupRef.current || !marqueeText2Ref.current || !marqueeText2DupRef.current) return;
  ...
  return (
    <group dispose={null}>
      ...
      <group>
        {
          Object.entries(nodes).filter(([key])=> key.startsWith("Texts")).map(([key, node])=>(
            <mesh  key={key} 
            geometry={node.geometry} material={textsMaterial} position={node.position} />
          ))
        }
      </group>
      <mesh
